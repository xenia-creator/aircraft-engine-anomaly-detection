(function () {
      ...
      options: {
          ...
          tooltip: {
            ...
            callbacks: {
              label: (item) => {
                ...
                return `Engine #${item.raw.unit_id} · error ${item.raw.y.toFixed(5)} · RUL ${item.raw.x}`;
              }
            }
          }
        },
        scales: {
          ...
          y: {
            min: 0,
            max: yMax,
            title: { display: true, text: 'Latest Reconstruction Error', color: CHART_COLORS.text },
            grid: baseGridOptions,
            ticks: { color: CHART_COLORS.text }
          }
        }
  ...
  async function init() {
    const res = await fetch('/api/fleet_overview');
